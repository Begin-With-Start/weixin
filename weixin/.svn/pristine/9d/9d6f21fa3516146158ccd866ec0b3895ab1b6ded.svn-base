...
function loadcjxm(){
	...
	$.post('/weixin/cj/Cjaction!getAllQgxm.action',null,function(data){
		for (var i = 0 ; i<data.length ;i++){
			...
			if(1 == xmid)
			{
				$div.find("div").eq(0).html("<image src='/weixin/pages/file/beizi_xiao.jpg'>");
			}
			else if(2 == xmid)
			{
				$div.find("div").eq(0).html("<image src='/weixin/pages/file/beizi_xiao.jpg'>");
			...
			{
				$div.find("div").eq(0).html("<a href = '/weixin/pages/1yuanmiaosha4.jsp?user="+user+"&xmid="+xmid+"'><image src='/weixin/pages/file/pingguo.jpg'></a>");
			}
			...
			if(4 == xmid)
			{
				$div.find("div").eq(1).find("div").eq(5).html(" <input  type='button' value='点击抢购' onclick='javascript:cj("+xmid+");return false;'>");
			}else
			{
			*/
			$div.find("div").eq(1).find("div").eq(5).html(" <input  type='button' value='已结束'>");
			//}
...
function cj(xmid){
	...
	if(1 == xmid)
	{
		window.location.href='/weixin/pages/1yuanmiaosha.jsp?user='+user+'&xmid='+xmid;
	}
	else if(2 == xmid)
	{
		window.location.href='/weixin/pages/1yuanmiaosha2.jsp?user='+user+'&xmid='+xmid;
	}
	else if(3 == xmid)
	{
		window.location.href='/weixin/pages/1yuanmiaosha3.jsp?user='+user+'&xmid='+xmid;
	}
	else if(4 == xmid)
	{
		window.location.href='/weixin/pages/1yuanmiaosha4.jsp?user='+user+'&xmid='+xmid;
	}
}
